...
- **Flagship WOW-01: Grand Expedition Atlas & Ecosystem Orrery**: Integrated in the Atlas view (`1` / Map dock / waystones). Features topographic elevation contours, an interactive Walking Route Surveyor calculating real path distances, leagues, steps, and river crossings, and the cosmic Ecosystem Orrery visualizing live source project condition and Git freshness as celestial orbits.
- **Project Beacons**: Every monitored settlement now has a physical read-only source beacon. Beacons expose branch, changed-path count, recorded upstream divergence, unresolved source-backed quests, and latest commit evidence. The Atlas and Orrery can open the same beacon view. These signals are descriptive telemetry only; Worldwalker does not score project quality or mutate the source.
