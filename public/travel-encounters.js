...
const conditionLine={sealed:'The place reads stable and sealed.',active:'The place is active.',open:'There is still open work here.',blocked:'Something source-backed is blocked here.',dormant:'The place is quiet, but not declared finished.',unknown:'The road enters fog. Worldwalker does not know the project state.',quiet:'No urgent source condition is recorded here.'};
export function encounterFor(game,seen={}){
  ...
  return {key,project:p,speaker:roleFor[p.id]||'Road Chronicler',portrait:'portrait_chronicler.png',lines:[`A traveler on the road from ${p.name} slows when they see you.`,conditionLine[p.condition]||`Worldwalker reads this place as ${String(p.condition||'unknown').toUpperCase()}.`,fact,'The encounter is Worldwalker fiction wrapped around source-backed facts.']};
}
