/**
 * BLACK BOX AQUARIUM v1.0 - Complete Systemic Alien Ecosystem
 * Local-only, procedural assets, WebAudio synthesis, no external AI.
 * Quality contract: title flow, tutorial, pause, restart, settings, audio, reduced-motion, save, reset-save, keyboard+pointer+touch.
 */
...
const TRAIT_DEFS = {
  size: {min:0.25,max:1.8},
  speed: {min:0.05,max:1},
  metabolism: {min:0.2,max:1},
  tempPref: {min:0,max:1},
  armor: {min:0,max:1},
  sensory: {min:0.1,max:1},
  reproRate: {min:0.1,max:1},
  aggression: {min:0,max:1},
  social: {min:0,max:1},
  biolum: {min:0,max:1},
  hue: {min:0,max:360},
  saturation: {min:0.4,max:1},
  bodyPlan: {options:5},
  finCount: {min:0,max:1},
  appendage: {min:0,max:1},
  shellType: {options:3},
  eyeCount: {min:0,max:1},
  mouthType: {options:5},
  marking: {options:5},
  feeding: {options:6},
};
...
class AudioMgr{
  constructor(){
    this.ctx=null; this.master=null; this.enabled=true; this.volume=0.25;
    this.ambienceNode=null; this.padNodes=[];
  }
  init(){
    if(this.ctx) return;
    try{
  ...
  startPad(){
    ...
    o1.start(); o2.start(); this.padNodes=[o1,o2,g,f];
    setInterval(()=>{
      if(!this.ctx) return;
  ...
  play(type){
    ...
    switch(type){
      ...
      case 'egg':
        osc.frequency.setValueAtTime(600,t); osc.frequency.linearRampToValueAtTime(300,t+0.3);
        gain.gain.setValueAtTime(0.12,t); gain.gain.linearRampToValueAtTime(0,t+0.3);
        osc.start(t); osc.stop(t+0.3); break;
    }
  }
  toggle(){ this.enabled=!this.enabled; if(this.master) this.master.gain.value=this.enabled?this.volume:0; }
}
...
class SpatialGrid{
  constructor(w,h,cols,rows){
    ...
    this.cellW=w/cols; this.cellH=h/rows; this.clear();
  }
  ...
  insert(o){ const k=this.key(o.x,o.y); this.cells[k].push(o); }
  query(x,y,r){
...
class Organism{
  constructor(genome, x,y, parentIds=[], generation=0, lineageId=null){
    ...
    this.observation=0; this.mutations={}; this.attachedTo=null; this.attachTimer=0; this.reproCooldown=0; this.isEgg=false; this.eggTimer=0;
  }
  get sizePx(){ return 8 + this.genome.size*18; }
  get sensoryPx(){ return 20 + this.genome.sensory*130; }
  update(env, grid, organisms, particles, dt, settings){
    if(!this.alive) return;
    if(this.isEgg){
      this.eggTimer-=dt;
      if(this.eggTimer<=0){ this.isEgg=false; this.energy=35; audio.play('egg'); }
      else return;
    }
    this.age+=dt; this.reproCooldown=Math.max(0,this.reproCooldown-dt); this.observation+=dt*0.2;
    ...
    let tempStress = tempDiff/18; if(this.genome.armor>0.6) tempStress*=0.6; if(this.genome.size>1.2) tempStress*=0.8;
    ...
    switch(this.role){
      case 'producer':
        ...
        this.energy+=photo; if(this.energy>100) this.energy=100;
        if(!settings.reducedMotion){ this.vx+=rand(-0.02,0.02); this.vy+=rand(-0.02,0.02); }
        env.oxygenProducer+= photo*0.12; env.nutrientConsumer+= photo*0.03; break;
      case 'grazer': this.seekFood(grid,'producer',dt,particles); break;
      case 'filter':
        const filt = env.nutrients/100 * dt*8; this.energy+= filt; env.nutrientConsumer+= filt*0.05; this.wander(dt,settings); break;
      case 'scavenger': this.seekFood(grid,'detritus',dt,particles); this.wander(dt,settings); break;
      case 'predator': this.seekFood(grid,'prey',dt,particles); break;
      case 'parasite': this.parasiteBehavior(grid,dt); break;
    }
    if(this.genome.social>0.5){
      const neighbors = grid.query(this.x,this.y,this.sensoryPx).filter(o=>o!==this && o.alive && !o.isEgg && o.role===this.role);
    ...
    if(this.genome.aggression>0.6){
      ...
      close.forEach(o=>{ const dx=this.x-o.x, dy=this.y-o.y, d=Math.hypot(dx,dy); if(d<20 && d>0.1){ this.vx+=dx/d*0.05; this.vy+=dy/d*0.05; } });
    }
    const maxSpeed = 0.3 + this.genome.speed*2.2; const spd=Math.hypot(this.vx,this.vy);
  ...
  seekFood(grid,type,dt,particles=null){
    ...
    if(type==='producer'){
      const candidates=grid.query(this.x,this.y,this.sensoryPx).filter(o=>o!==this && o.alive && !o.isEgg && o.role==='producer');
      ...
      if(target){
        ...
        if(d<this.sizePx+target.sizePx+6){
          ...
          audio.play('feed');
        }else{ this.vx+=dx/d*0.08*this.genome.speed; this.vy+=dy/d*0.08*this.genome.speed; }
      }else this.wander(dt);
    }else if(type==='prey'){
      const candidates=grid.query(this.x,this.y,this.sensoryPx).filter(o=>o!==this && o.alive && !o.isEgg && o.role!=='producer' && o.role!=='predator' && o.genome.size < this.genome.size*0.9);
      for(const c of candidates){ const d=Math.hypot(c.x-this.x,c.y-this.y); if(d<bestDist){bestDist=d; target=c;} }
      if(target){
        const dx=target.x-this.x, dy=target.y-this.y, d=Math.hypot(dx,dy);
        if(d<this.sizePx+target.sizePx+4){
          ...
          if(success){ this.energy+= 18 * (0.5+target.genome.size); target.health-= 50; if(target.health<=0) target.die(particles); audio.play('feed'); }
          else{ target.vx+=dx/d*1.2; target.vy+=dy/d*1.2; this.vx-=dx/d*0.3; }
        }else{ this.vx+=dx/d*0.1*this.genome.speed; this.vy+=dy/d*0.1*this.genome.speed; }
      }else this.wander(dt);
    }else if(type==='detritus'){
      if(!particles) return; let best=null; let bd=9999;
      ...
      if(best){
        ...
        if(d<this.sizePx+4){ this.energy+=10; best.life=0; }else{ this.vx+=dx/d*0.06; this.vy+=dy/d*0.06; }
      }else this.wander(dt);
    }
  }
  parasiteBehavior(grid,dt){
    if(this.attachedTo && this.attachedTo.alive && !this.attachedTo.isEgg){
      ...
      const candidates=grid.query(this.x,this.y,this.sensoryPx).filter(o=>o!==this && o.alive && !o.isEgg && o.role!=='producer' && o.role!=='parasite' && o.genome.size>this.genome.size*0.7);
      let best=null; let bd=9999;
      for(const c of candidates){ const d=Math.hypot(c.x-this.x,c.y-this.y); if(d<bd){bd=d; best=c;} }
      if(best && bd<40){ this.attachedTo=best; this.vx=0; this.vy=0; }
      else if(best){ const dx=best.x-this.x, dy=best.y-this.y, d=Math.hypot(dx,dy); this.vx+=dx/d*0.08; this.vy+=dy/d*0.08; }
      else this.wander(dt);
    }
  }
  die(particles){
    if(!this.alive) return; this.alive=false; this.health=0;
    if(particles){
  ...
  canReproduce(){
    ...
    return this.alive && !this.isEgg && this.energy>threshold && this.reproCooldown<=0 && this.age> (this.role==='producer'?3:6) && this.health>45;
  }
  reproduce(partner=null){
    ...
    child.mutations=mutations; child.energy=30;
    // egg stage: 2-4 sec
    child.isEgg=true; child.eggTimer=2+rand(0,2);
    this.energy-=28; if(partner) partner.energy-=20;
...
class Environment{
  constructor(){
    this.temperature=20; this.oxygen=55; this.ph=7.0; this.light=60; this.nutrients=50; this.contamination=5;
    this.oxygenProducer=0; this.nutrientConsumer=0;
    this.history={temp:[], oxygen:[], ph:[], light:[], nutrients:[], contam:[]};
    this.time=0;
  }
  update(dt, controls){
    this.time+=dt;
...
class ResearchMgr{
  constructor(){
    this.traitsDiscovered=new Set(); this.speciesCatalog=new Map();
    this.observations=0; this.totalBirths=0; this.symbiosisObserved=false;
  }
  observe(org, dt){
    ...
    for(const k in thresholds){ if(org.observation>thresholds[k]) this.traitsDiscovered.add(k); }
    if(org.role==='parasite' && org.attachedTo && org.genome.social>0.6 && org.attachedTo.health>70) this.symbiosisObserved=true;
  }
  catalog(organisms){
    this.speciesCatalog.clear();
    for(const o of organisms){
      if(!o.alive || o.isEgg) continue;
      ...
      entry.count++; entry.avgSize+=o.genome.size; entry.avgTemp+=o.genome.tempPref;
      if(entry.examples.length<3) entry.examples.push(o);
    }
    for(const v of this.speciesCatalog.values()){ if(v.count>0){ v.avgSize/=v.count; v.avgTemp/=v.count; } }
  }
}
...
class CampaignMgr{
  constructor(){
    this.milestones=[
      ...
      {id:'m3', title:'COLD FORGE', desc:'Breed cold-resistant lineage (tempPref <0.25) surviving at <15°C for 30s', check:(sim)=>sim.environment.temperature<15 && sim.organisms.some(o=>o.alive && !o.isEgg && o.genome.tempPref<0.25), progress:0, required:30, done:false},
    ...
    this.completed=0; this.endingTriggered=false;
  }
  update(sim, dt){
    for(const m of this.milestones){
      ...
      }else{ if(!m.instant) { m.progress=Math.max(0,m.progress-dt*0.3); m.active=false; } }
    }
  }
}
...
class EventMgr{
  constructor(){
    ...
    this.active=null; this.timer=0; this.nextEvent= rand(45,90);
  }
  update(sim, dt){
    this.nextEvent-=dt;
    if(this.active){
  ...
  triggerRandom(sim){
    ...
    const vig=document.getElementById('contamVignette'); if(vig){ vig.style.opacity='0.6'; setTimeout(()=>{vig.style.opacity='0';}, ev.duration*1000); }
  }
  triggerById(id, sim){
...
class Simulation{
  constructor(){
    ...
    this.organisms=[]; this.particles=[]; this.habitats=[];
    this.grid=new SpatialGrid(CONFIG.TANK_W, CONFIG.TANK_H, CONFIG.GRID_COLS, CONFIG.GRID_ROWS);
    this.research=new ResearchMgr(); this.campaign=new CampaignMgr(); this.events=new EventMgr();
    this.selected=null; this.breedSlots=[null,null]; this.paused=false; this.speed=1; this.time=0;
    ...
    this.logEntries=[]; this.mode='campaign'; this.bubbleTimer=0; this.autoSaveTimer=0;
    this.settings={volume:25, reducedMotion:false, lowGraphics:false, autoSave:true, showTooltips:true};
    this.tutorial={active:false, step:0, steps:[
      {title:'WELCOME TO THE TANK', text:'12 drops of alien ocean. Your job: keep them alive. Evolution is not a feature, it is a consequence. Click creatures to inspect. Observation unlocks traits.'},
      {title:'ENVIRONMENT IS SELECTION', text:'Use left panel sliders: Temperature, Light, O₂ Pump, pH, Nutrients, Filter. Changing temp selects for cold/hot-resistant lineages. Try lowering temp to 10°C and watch avg tempPref shift.'},
      {title:'FEEDING & ECOLOGY', text:'Producers need light+nutrients and make O₂. Grazers eat producers. Filters eat nutrients. Predators hunt. Parasites attach. Overgrazing crashes producers — seed more if needed. Feed algae wafers for producers.'},
      {title:'LINEAGE & RESEARCH', text:'Inspector shows ID, generation, lineage, mutations (*) with from→to. Lineage browser shows ancestry chain. Research notebook catalogs species, unlocks traits, graphs population. Milestones track progress.'},
      {title:'SELECTIVE BREEDING', text:'Isolation tank: click organism → Isolate → Breed Tank → add second → Attempt Cross. Offspring inherit with mutation. Eggs hatch after 2-4s. Use to breed cold-resistant or bioluminescent lines.'},
      {title:'EVENTS & DISCOVERY', text:'Random events: O₂ crash, fungal bloom, heater failure, spores, repro surge, malfunction. Final discovery: Chorus Mind — biolum>0.85 + social>0.75 + sensory>0.75 schooling 5. You did not design it, but you understand why it evolved. Good luck.'},
    ]};

    this.loadSettings();
    this.bindUI();
    this.resize();
    this.renderTitleCanvas();
    window.addEventListener('resize',()=>this.resize());
    // auto title animation
    setInterval(()=>this.renderTitleCanvas(), 100);
  }
  loadSettings(){
    try{
      const raw=localStorage.getItem('blackbox_settings');
      if(raw){ const s=JSON.parse(raw); Object.assign(this.settings,s); }
    }catch{}
    // apply to UI later in bindUI
  }
  saveSettings(){
    localStorage.setItem('blackbox_settings', JSON.stringify(this.settings));
    audio.setVolume(this.settings.volume/100*0.5);
    document.body.classList.toggle('reduced-motion', this.settings.reducedMotion);
  }
  renderTitleCanvas(){
    const c=this.titleCanvas; if(!c) return; const ctx=c.getContext('2d');
    const w=c.width, h=c.height;
    ctx.clearRect(0,0,w,h);
    // subtle alien ocean drops
    const time=Date.now()*0.001;
    for(let i=0;i<12;i++){
      const x= (w/2) + Math.cos(time*0.3+i)* (120+i*8) + Math.sin(i)*20;
      const y= h*0.5 + Math.sin(time*0.2+i*1.1)*30;
      const hue= 160 + i*15 + Math.sin(time+i)*10;
      ctx.fillStyle=`hsla(${hue},80%,60%,0.15)`;
      ctx.beginPath(); ctx.arc(x,y, 8+Math.sin(time+i)*3,0,Math.PI*2); ctx.fill();
      // inner
      ctx.fillStyle=`hsla(${hue},90%,70%,0.4)`;
      ctx.beginPath(); ctx.arc(x,y, 3,0,Math.PI*2); ctx.fill();
    }
    // probe silhouette
    ctx.strokeStyle='rgba(108,242,194,0.2)'; ctx.lineWidth=1;
    ctx.beginPath(); ctx.arc(w/2, h/2, 40+Math.sin(time)*2,0,Math.PI*2); ctx.stroke();
  }
  resize(){
  ...
  bindUI(){
    const bind=(id, key, display, fmt=v=>v)=>{
      const el=document.getElementById(id); const out=document.getElementById(display);
      if(!el) return;
    ...
    document.getElementById('resetBtn')?.addEventListener('click',()=>{ if(confirm('Reset tank? This clears current organisms but not saved file.')) this.reset(); });
    ...
    document.getElementById('clearSaveBtn')?.addEventListener('click',()=>{ if(confirm('Clear saved tank? This deletes localStorage save.')){ localStorage.removeItem('blackbox_save'); this.logEvent('Saved tank cleared','system'); } });
    document.getElementById('exportSaveBtn')?.addEventListener('click',()=>this.save());
    document.getElementById('failureRestart')?.addEventListener('click',()=>{ document.getElementById('failureOverlay').classList.add('hidden'); this.reset(); });
    document.getElementById('failureLoad')?.addEventListener('click',()=>{ document.getElementById('failureOverlay').classList.add('hidden'); this.load(); });

    // settings controls
    const volSlider=document.getElementById('volSlider'); const volVal=document.getElementById('volVal');
    if(volSlider){ volSlider.value=this.settings.volume; if(volVal) volVal.textContent=this.settings.volume+'%'; volSlider.addEventListener('input',()=>{ this.settings.volume=parseInt(volSlider.value); if(volVal) volVal.textContent=this.settings.volume+'%'; this.saveSettings(); }); }
    const rm=document.getElementById('reducedMotion'); if(rm){ rm.checked=this.settings.reducedMotion; rm.addEventListener('change',()=>{ this.settings.reducedMotion=rm.checked; this.saveSettings(); }); }
    const lg=document.getElementById('lowGraphics'); if(lg){ lg.checked=this.settings.lowGraphics; lg.addEventListener('change',()=>{ this.settings.lowGraphics=lg.checked; this.saveSettings(); }); }
    const as=document.getElementById('autoSave'); if(as){ as.checked=this.settings.autoSave; as.addEventListener('change',()=>{ this.settings.autoSave=as.checked; this.saveSettings(); }); }
    const tt=document.getElementById('showTooltips'); if(tt){ tt.checked=this.settings.showTooltips; tt.addEventListener('change',()=>{ this.settings.showTooltips=tt.checked; this.saveSettings(); }); }

    // tutorial
    document.getElementById('tutorialNext')?.addEventListener('click',()=>this.tutorialNext());
    document.getElementById('tutorialPrev')?.addEventListener('click',()=>this.tutorialPrev());
    document.getElementById('tutorialSkip')?.addEventListener('click',()=>this.tutorialEnd());
    ...
    this.canvas.addEventListener('click',(e)=>{ const p=getPos(e); this.selectAt(p.x,p.y); });
    this.canvas.addEventListener('mousemove',(e)=>{ const p=getPos(e); this.hoverAt(p.x,p.y,e); });
    this.canvas.addEventListener('touchstart',(e)=>{ e.preventDefault(); const p=getPos(e); this.selectAt(p.x,p.y); }, {passive:false});
    this.canvas.addEventListener('touchmove',(e)=>{ e.preventDefault(); const p=getPos(e); this.hoverAt(p.x,p.y,{clientX:p.clientX, clientY:p.clientY}); }, {passive:false});

    this.breedCanvas.addEventListener('click',()=>{ if(this.selected) this.addToBreed(this.selected); });
    this.breedCanvas.addEventListener('touchstart',(e)=>{ e.preventDefault(); if(this.selected) this.addToBreed(this.selected); }, {passive:false});
    ...
    window.addEventListener('keydown',(e)=>{
      ...
      else if(e.code==='Tab'){ e.preventDefault(); this.cycleSelection(); }
      else if(e.code==='KeyS' && !e.ctrlKey){ this.save(); }
      else if(e.code==='KeyL'){ this.load(); }
      else if(e.code==='KeyR'){ if(confirm('Reset?')) this.reset(); }
      else if(e.code==='KeyM'){ document.getElementById('manualOverlay').classList.toggle('hidden'); }
      else if(e.code==='Comma'){ document.getElementById('settingsOverlay').classList.toggle('hidden'); }
  ...
  setSpeed(s){
    this.speed=s;
    document.querySelectorAll('[data-speed]').forEach(x=>{ x.classList.toggle('active', parseFloat(x.dataset.speed)===s); });
  }
  cycleSelection(){
    const alive=this.organisms.filter(o=>o.alive && !o.isEgg);
    if(alive.length===0) return;
    if(!this.selected){ this.selected=alive[0]; }
    else{
      const idx=alive.indexOf(this.selected);
      this.selected=alive[(idx+1)%alive.length];
    }
    this.updateInspector();
    if(this.selected){
      document.getElementById('selectionRing').style.display='block';
      document.getElementById('selectionRing').style.left=this.selected.x+'px';
      document.getElementById('selectionRing').style.top=this.selected.y+'px';
    }
  }
  startGame(mode){
    this.mode=mode;
    document.getElementById('titleScreen').classList.add('hidden');
    audio.init(); audio.setVolume(this.settings.volume/100*0.5);
    ...
    const ml=document.getElementById('manualMilestones'); if(ml){ ml.innerHTML=''; this.campaign.milestones.forEach(m=>{ const li=document.createElement('li'); li.textContent=`${m.title}: ${m.desc}`; ml.appendChild(li); }); }
    if(mode==='campaign' && !localStorage.getItem('blackbox_tutorial_done')){
      this.tutorialStart();
    }
  }
  tutorialStart(){ this.tutorial.active=true; this.tutorial.step=0; this.showTutorialStep(); document.getElementById('tutorialOverlay').classList.remove('hidden'); }
  showTutorialStep(){
    const s=this.tutorial.steps[this.tutorial.step];
    document.getElementById('tutorialTitle').textContent=s.title;
    document.getElementById('tutorialText').textContent=s.text;
    document.getElementById('tutorialStep').textContent=`${this.tutorial.step+1}/${this.tutorial.steps.length}`;
  }
  tutorialNext(){ this.tutorial.step++; if(this.tutorial.step>=this.tutorial.steps.length){ this.tutorialEnd(); }else{ this.showTutorialStep(); } }
  tutorialPrev(){ if(this.tutorial.step>0){ this.tutorial.step--; this.showTutorialStep(); } }
  tutorialEnd(){ this.tutorial.active=false; document.getElementById('tutorialOverlay').classList.add('hidden'); localStorage.setItem('blackbox_tutorial_done','1'); }
  seedInitial(){
    this.organisms=[]; this.particles=[]; this.habitats=[];
    for(let i=0;i<CONFIG.INITIAL_COUNT;i++){
      const g=randomGenome();
      if(i<8) g.feeding=0; else if(i<10) g.feeding=1; else if(i<11) g.feeding=2; else g.feeding=choice([2,3,4]);
  ...
  reset(){
    ...
    this.popHistory={producer:[], grazer:[], filter:[], scavenger:[], predator:[], parasite:[], total:[]};
    this.traitHistory={avgTempPref:[], avgBiolum:[], avgSocial:[], avgSize:[]};
    this.research=new ResearchMgr(); this.campaign=new CampaignMgr(); this.logEntries=[]; this.selected=null; this.breedSlots=[null,null];
    this.seedInitial(); this.logEvent('TANK RESET // NEW SAMPLE INTRODUCED','system');
    document.getElementById('failureOverlay').classList.add('hidden');
  }
  togglePause(){
    this.paused=!this.paused;
    const btn=document.getElementById('pauseBtn'); if(btn) btn.textContent=this.paused?'▶':'⏸';
    this.logEvent(this.paused?'SIMULATION PAUSED — evolution frozen':'SIMULATION RESUMED','system');
  }
  countRole(role){ return this.organisms.filter(o=>o.alive && !o.isEgg && o.role===role).length; }
  aliveCount(){ return this.organisms.filter(o=>o.alive && !o.isEgg).length; }
  spawnRandom(n=1, mutate=false){
    for(let i=0;i<n;i++){
      let g=randomGenome();
      if(mutate && this.organisms.length>0){
        const parent=choice(this.organisms.filter(o=>o.alive && !o.isEgg));
  ...
  applyFood(type){
    ...
    switch(type){
      case 'algae':
        ...
        this.environment.nutrients=Math.min(100,this.environment.nutrients+5); break;
      case 'protein':
        for(let i=0;i<8;i++) this.particles.push({x:rand(0,CONFIG.TANK_W), y:rand(0,CONFIG.TANK_H), vx:rand(-0.3,0.3), vy:rand(-0.2,0.2), type:'protein', life:18, size:5}); break;
      case 'detritus':
        for(let i=0;i<15;i++) this.particles.push({x:rand(0,CONFIG.TANK_W), y:rand(0,CONFIG.TANK_H), vx:rand(-0.2,0.2), vy:rand(0.1,0.4), type:'detritus', life:25, size:3}); break;
      case 'nutrient': this.environment.nutrients=Math.min(100,this.environment.nutrients+18); this.logEvent('Nutrient burst injected','env'); break;
      case 'clean': this.environment.contamination=Math.max(0,this.environment.contamination-22); this.logEvent('Decontaminant applied','env'); break;
      case 'o2': this.environment.oxygen=Math.min(100,this.environment.oxygen+20); break;
    }
  }
  applyHabitat(type){
    if(type==='clear'){ this.habitats=[]; return; }
    if(type==='light'){ this.controls.light=Math.min(100,this.controls.light+10); return; }
  ...
  applyIntro(type){
    switch(type){
      ...
      case 'grazer': { let g=randomGenome(); g.feeding=1; for(let i=0;i<4;i++){ const o=new Organism({...g}, rand(100,CONFIG.TANK_W-100), rand(100,CONFIG.TANK_H-100)); this.organisms.push(o);} } break;
  ...
  selectAt(x,y){
    ...
    for(const o of this.organisms){ if(!o.alive || o.isEgg) continue; const d=Math.hypot(o.x-x,o.y-y); if(d<bestDist){bestDist=d; best=o;} }
    this.selected=best; this.updateInspector();
    if(best){
      const ring=document.getElementById('selectionRing'); if(ring){ ring.style.display='block'; ring.style.left=best.x+'px'; ring.style.top=best.y+'px'; }
      this.research.observe(best,2);
    }else{ const ring=document.getElementById('selectionRing'); if(ring) ring.style.display='none'; }
  }
  hoverAt(x,y,e){
    if(!this.settings.showTooltips) return;
    let found=null;
    for(const o of this.organisms){ if(!o.alive || o.isEgg) continue; if(Math.hypot(o.x-x,o.y-y)<o.sizePx+6){found=o; break;} }
    const tt=document.getElementById('tooltip'); if(!tt) return;
    if(found){
      tt.style.display='block';
      tt.style.left=(e.clientX+12)+'px'; tt.style.top=(e.clientY-10)+'px';
      tt.innerHTML=`<b>${found.role.toUpperCase()}</b> gen:${found.generation}${found.isEgg?' EGG':''}<br>size:${found.genome.size.toFixed(2)} tempPref:${(found.genome.tempPref*30+5).toFixed(1)}°C<br>energy:${found.energy.toFixed(0)} health:${found.health.toFixed(0)}`;
  ...
  updateBreedCanvas(){
    const ctx=this.breedCtx; if(!ctx) return;
    ctx.clearRect(0,0,240,120); ctx.fillStyle='#081419'; ctx.fillRect(0,0,240,120);
    ...
    const info=document.getElementById('breedInfo'); if(info) info.textContent= this.breedSlots[0] && this.breedSlots[1] ? 'Ready to cross' : this.breedSlots[0] ? 'Select second parent' : 'Drop 2 organisms';
  }
  attemptBreed(){
  ...
  checkChorus(){
    const candidates=this.organisms.filter(o=>o.alive && !o.isEgg && o.genome.biolum>0.85 && o.genome.social>0.75 && o.genome.sensory>0.75);
    for(const c of candidates){
      const neighbors=this.grid.query(c.x,c.y,90).filter(o=>o.alive && !o.isEgg && o!==c && Math.abs(o.genome.biolum-c.genome.biolum)<0.15 && Math.abs(o.genome.hue-c.genome.hue)<30);
  ...
  triggerEnding(){
    ...
    const avgSize=this.organisms.filter(o=>o.alive && !o.isEgg).reduce((s,o)=>s+o.genome.size,0)/Math.max(1,this.aliveCount());
  ...
  logEvent(text, type='info'){
    ...
    if(type==='milestone' || type==='event'){
      const bar=document.getElementById('alertBar'); if(bar){ bar.textContent=text; bar.style.color=type==='milestone'?'#6cf2c2':'#ffde7a'; setTimeout(()=>{bar.textContent='';},6000); }
    }
  }
  showMilestone(m){
    const el=document.getElementById('milestoneTitle'); if(el) el.textContent=m.title;
    const desc=document.getElementById('milestoneDesc'); if(desc) desc.textContent=m.desc;
  ...
  loop(timestamp){
    ...
    dt=Math.min(dt,0.1); dt*=this.speed;
    if(!this.paused){
      this.time+=dt;
      ...
      for(const o of this.organisms){
        if(!o.alive) continue;
        o.update(this.environment, this.grid, this.organisms, this.particles, dt, this.settings);
        if(o.canReproduce() && this.aliveCount()+newBorns.length < CONFIG.POP_CAP){
          if(Math.random()< o.genome.reproRate*0.04*dt*20){
            ...
            if(o.genome.social>0.5){
              const near=this.grid.query(o.x,o.y,50).filter(n=>n!==o && n.alive && !n.isEgg && n.role===o.role && n.canReproduce());
        ...
        if(o.alive && !o.isEgg) this.research.observe(o, dt*0.1);
      }
      ...
      if(this.bubbleTimer<=0){
        this.bubbleTimer=rand(0.2,0.8);
        if(!this.settings.lowGraphics) this.particles.push({x:rand(0,CONFIG.TANK_W), y:CONFIG.TANK_H-5, vx:rand(-0.2,0.2), vy:rand(-0.8,-0.3), type:'bubble', life:rand(3,7), size:rand(1,3)});
        if(Math.random()<0.3) audio.play('bubble');
      }
      ...
      if(this.aliveCount()===0){
        const fail=document.getElementById('failureOverlay');
        if(fail && fail.classList.contains('hidden')){ fail.classList.remove('hidden'); this.logEvent('TANK COLLAPSE — all organisms perished','warn'); }
      }
      this.autoSaveTimer+=dt;
      if(this.settings.autoSave && this.autoSaveTimer>30){ this.autoSaveTimer=0; this.saveToStorage(); }

      if(this.time%0.3<dt) this.updateUI();
    }
    this.render();
  ...
  updateUI(){
    const pc=document.getElementById('popCount'); if(pc) pc.textContent=this.aliveCount();
    const cap=document.getElementById('popCap'); if(cap) cap.textContent=CONFIG.POP_CAP;
    const o2=document.getElementById('o2Read'); if(o2) o2.textContent=this.environment.oxygen.toFixed(1);
    const cr=document.getElementById('contamRead'); if(cr) cr.textContent=this.environment.contamination.toFixed(1);
    const ph=document.getElementById('phRead'); if(ph) ph.textContent=this.environment.ph.toFixed(2);
    const perf=document.getElementById('perfIndicator'); if(perf) perf.textContent=`${this.organisms.length} entities`;
    const bar=document.getElementById('populationBar'); if(bar){
      bar.innerHTML='';
    ...
    this.renderSpecies(); this.renderTraits(); this.renderMilestones(); this.renderGraphs(); this.renderEnvGraph();
    if(this.selected) this.updateInspector();
  }
  renderSpecies(){
    const el=document.getElementById('speciesList'); if(!el) return; el.innerHTML='';
    ...
    for(const [key, data] of sorted){
      ...
      div.innerHTML=`<b style="color:${ROLE_COLOR[data.role]}">${data.role.toUpperCase()}</b> x${data.count} <span class="dim">bp:${data.bodyPlan} avgSize:${data.avgSize.toFixed(2)} temp:${(data.avgTemp*30+5).toFixed(1)}°C</span><br><small>${data.examples.map(o=>o.id).join(', ')}</small>`;
      div.addEventListener('click',()=>{ if(data.examples[0]){ this.selected=data.examples[0]; this.updateInspector(); }});
      el.appendChild(div);
    }
    if(sorted.length===0) el.innerHTML='<span class="dim">No species cataloged yet. Observe.</span>';
  }
  renderTraits(){
    const el=document.getElementById('traitList'); if(!el) return; el.innerHTML='';
    for(const t of Object.keys(TRAIT_DEFS)){
  ...
  renderMilestones(){
    const el=document.getElementById('milestoneList'); if(!el) return; el.innerHTML='';
  ...
  renderGraphs(){
    const ctx=this.popCtx; if(!ctx) return; const w=this.popCanvas.width, h=this.popCanvas.height;
  ...
  renderEnvGraph(){
    const ctx=this.envCtx; if(!ctx) return; const w=this.envCanvas.width, h=this.envCanvas.height;
    ctx.clearRect(0,0,w,h); ctx.fillStyle='#081419'; ctx.fillRect(0,0,w,h);
  ...
  render(){
    const ctx=this.ctx; const W=CONFIG.TANK_W, H=CONFIG.TANK_H;
    ctx.clearRect(0,0,W,H);
    ...
    for(const p of this.particles){
      if(this.settings.lowGraphics && p.type!=='bubble' && Math.random()<0.5) continue;
      ...
      else if(p.type==='egg'){ ctx.fillStyle=`hsla(${p.hue||180},80%,70%,0.8)`; ctx.strokeStyle='rgba(255,255,255,0.6)'; ctx.lineWidth=1; ctx.beginPath(); ctx.ellipse(p.x,p.y,p.size*1.2,p.size,0,0,Math.PI*2); ctx.fill(); ctx.stroke(); }
      ctx.restore();
    }
    const alive=this.organisms.filter(o=>o.alive).sort((a,b)=>a.y-b.y);
    for(const org of alive){ this.drawOrganism(ctx, org, this.time); }
    if(this.selected && this.selected.alive){
      const ring=document.getElementById('selectionRing'); if(ring){ ring.style.left=this.selected.x+'px'; ring.style.top=this.selected.y+'px'; }
    }
    const contamEl=document.getElementById('contamVignette'); if(contamEl) contamEl.style.opacity= (this.environment.contamination/100)*0.6;
  }
  drawOrganism(ctx, org, time){
    const g=org.genome;
    if(org.isEgg){
      ctx.save(); ctx.translate(org.x, org.y);
      ctx.fillStyle=`hsla(${g.hue},70%,65%,0.9)`; ctx.strokeStyle='rgba(255,255,255,0.7)'; ctx.lineWidth=1;
      const pulse=1+Math.sin(time*3)*0.1;
      ctx.scale(pulse,pulse);
      ctx.beginPath(); ctx.ellipse(0,0,6,8,0,0,Math.PI*2); ctx.fill(); ctx.stroke();
      ctx.fillStyle='rgba(255,255,255,0.5)'; ctx.beginPath(); ctx.arc(-1.5,-2,1.5,0,Math.PI*2); ctx.fill();
      ctx.restore(); return;
    }
    ctx.save(); ctx.translate(org.x, org.y);
    const angle=this.settings.reducedMotion?0:Math.atan2(org.vy, org.vx);
    ctx.rotate(angle); const scale=g.size; ctx.scale(scale,scale);
    if(g.biolum>0.3 && !this.settings.lowGraphics){
      ctx.shadowBlur= 8+ g.biolum*20; ctx.shadowColor=`hsla(${g.hue},${g.saturation*100}%,60%,${0.4+g.biolum*0.5})`;
    }
    const bodyColor=`hsl(${g.hue},${g.saturation*100}%,${45+g.armor*10}%)`;
    ctx.fillStyle=bodyColor; ctx.strokeStyle=`hsl(${g.hue},${g.saturation*100}%,30%)`; ctx.lineWidth=0.8;
    const bp=g.bodyPlan;
    if(bp===0){
      ...
      const tailW= 8+ g.finCount*6; ctx.beginPath(); ctx.moveTo(-14,0); ctx.lineTo(-14-tailW, -6); ctx.lineTo(-14-tailW,6); ctx.closePath(); ctx.fill(); ctx.stroke();
    }else if(bp===2){
      ctx.lineCap='round'; ctx.lineWidth=6; ctx.strokeStyle=bodyColor; ctx.beginPath();
      for(let i=0;i<3;i++){ const off=this.settings.reducedMotion?0:Math.sin(time*2+i)*2; ctx.moveTo(-12+i*8,off); ctx.lineTo(-4+i*8,off); ctx.stroke(); }
      ...
      for(let i=-1;i<=1;i++){
        const ly=this.settings.reducedMotion?0:Math.sin(time*3+i)*2;
        ctx.beginPath(); ctx.moveTo(i*4,4); ctx.lineTo(i*8+ (i*3),10+ly); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(i*4,-4); ctx.lineTo(i*8+(i*3),-10+ly); ctx.stroke();
      }
    }else{
    ...
    if(g.shellType>0 && g.armor>0.35){
      ctx.fillStyle=`hsla(${g.hue+20},30%,75%,${0.5+g.armor*0.4})`; ctx.strokeStyle=`hsla(${g.hue},20%,40%,0.8)`; ctx.lineWidth=1;
      if(g.shellType===1){ ctx.beginPath(); ctx.ellipse(0,0,11,9,0,0,Math.PI*2); ctx.fill(); ctx.stroke(); }
      else{ ctx.beginPath(); ctx.arc(0,0,10,0,Math.PI*2); ctx.fill(); ctx.stroke(); for(let i=0;i<6;i++){ const a=i/6*Math.PI*2; ctx.beginPath(); ctx.moveTo(Math.cos(a)*10,Math.sin(a)*10); ctx.lineTo(Math.cos(a)*14,Math.sin(a)*14); ctx.stroke(); } }
    }
    ...
    ctx.fillStyle='#1a0a0a'; const mx=10, my=0;
    ...
    if(g.marking>0){
      ...
      if(g.marking===1){ for(let i=-8;i<8;i+=4){ ctx.beginPath(); ctx.moveTo(i,-6); ctx.lineTo(i,6); ctx.stroke(); } }
      else if(g.marking===2){ ctx.fillStyle=`hsla(${(g.hue+60)%360},80%,60%,0.8)`; for(let i=0;i<3;i++){ ctx.beginPath(); ctx.arc(-6+i*6, rand(-4,4),2,0,Math.PI*2); ctx.fill(); } }
    ...
    ctx.restore();
    if(org.health<95){ ctx.fillStyle='rgba(0,0,0,0.5)'; ctx.fillRect(org.x-10, org.y-org.sizePx-8,20,3); ctx.fillStyle= org.health>50 ? '#6cf2a2' : '#ff6b6b'; ctx.fillRect(org.x-10, org.y-org.sizePx-8, 20*(org.health/100),3); }
    if(org.energy<90){ ctx.fillStyle='rgba(0,0,0,0.4)'; ctx.fillRect(org.x-10, org.y-org.sizePx-12,20,2); ctx.fillStyle='#8aa8ff'; ctx.fillRect(org.x-10, org.y-org.sizePx-12,20*(org.energy/100),2); }
    if(org.attachedTo && org.attachedTo.alive){ ctx.strokeStyle=`hsla(${g.hue},70%,60%,0.4)`; ctx.lineWidth=0.8; ctx.setLineDash([2,2]); ctx.beginPath(); ctx.moveTo(org.x,org.y); ctx.lineTo(org.attachedTo.x,org.attachedTo.y); ctx.stroke(); ctx.setLineDash([]); }
  }
  drawOrganismSmall(ctx, org, x,y, angle){
    ctx.save(); ctx.translate(x,y); ctx.rotate(angle); ctx.scale(0.7,0.7); ctx.translate(-org.x,-org.y); this.drawOrganism(ctx, org, 0); ctx.restore();
  }
  updateInspector(){
    const empty=document.getElementById('inspectorEmpty'); const content=document.getElementById('inspectorContent');
    if(!this.selected || !this.selected.alive){ if(empty) empty.style.display='block'; if(content) content.classList.add('hidden'); return; }
    if(empty) empty.style.display='none'; if(content) content.classList.remove('hidden');
    const org=this.selected; const ictx=this.inspectorCtx; if(!ictx) return;
    ...
    ictx.fillStyle=`hsl(${g.hue},${g.saturation*100}%,55%)`; ictx.beginPath(); ictx.ellipse(0,0,14,8,0,0,Math.PI*2); ictx.fill();
    ictx.fillStyle=ROLE_COLOR[org.role]; ictx.beginPath(); ictx.arc(-12,-8,3,0,Math.PI*2); ictx.fill();
    ictx.fillStyle='#cde6df'; ictx.font='8px monospace'; ictx.fillText(org.role.toUpperCase()+(org.isEgg?' EGG':''),-20,20);
    ictx.restore();
    const meta=document.getElementById('inspectorMeta'); if(meta) meta.innerHTML=`ID: ${org.id} | GEN: ${org.generation} | LINEAGE: ${org.lineageId}<br>AGE: ${org.age.toFixed(1)}s | ENERGY: ${org.energy.toFixed(0)} | HEALTH: ${org.health.toFixed(0)}${org.isEgg?` | HATCH IN ${org.eggTimer.toFixed(1)}s`:''}<br>ROLE: ${org.role} | SIZE: ${g.size.toFixed(2)} | OBS: ${org.observation.toFixed(1)}s`;
    const traitsDiv=document.getElementById('inspectorTraits'); if(traitsDiv){
      traitsDiv.innerHTML='';
      for(const k in TRAIT_DEFS){
        const disc=this.research.traitsDiscovered.has(k); const val=g[k]; const isMut=org.mutations[k];
    ...
    const linDiv=document.getElementById('inspectorLineage'); if(linDiv) linDiv.innerHTML=`Parents: ${org.parentIds.join(', ')||'ORIGIN SAMPLE'}<br>Mutations: ${Object.keys(org.mutations).length>0?Object.entries(org.mutations).map(([k,v])=>`${k}: ${typeof v.from==='number'?v.from.toFixed(2):v.from}→${typeof v.to==='number'?v.to.toFixed(2):v.to}`).join(', '):'none (clone)'}<br>Lineage depth: ${org.generation}`;
    this.drawLineage(org);
  }
  drawLineage(org){
    const ctx=this.lineageCtx; if(!ctx) return; const w=this.lineageCanvas.width, h=this.lineageCanvas.height;
    ctx.clearRect(0,0,w,h); ctx.fillStyle='#081419'; ctx.fillRect(0,0,w,h);
    const chain=[]; let cur=org; let depth=0;
    while(cur && depth<12){ chain.push(cur); if(cur.parentIds.length>0){ const pid=cur.parentIds[0]; const found=this.organisms.find(o=>o.id===pid); cur=found||null; }else cur=null; depth++; }
    chain.reverse();
    ctx.strokeStyle='#1f3a44'; ctx.lineWidth=1;
    chain.forEach((o,i)=>{
      const x= 20 + (i/(Math.max(1,chain.length-1)))*(w-40); const y= h/2 + Math.sin(i)*10;
      if(i<chain.length-1){ const nx=20+((i+1)/(Math.max(1,chain.length-1)))*(w-40); ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(nx,h/2+Math.sin(i+1)*10); ctx.stroke(); }
      ...
      if(Object.keys(o.mutations).length>0){ ctx.fillStyle='#ffde7a'; ctx.beginPath(); ctx.arc(x,y-10,2,0,Math.PI*2); ctx.fill(); }
    });
    ctx.fillStyle='#7a9a93'; ctx.font='9px monospace'; ctx.fillText(`Lineage: ${org.lineageId} depth ${org.generation}`,4,12);
    const info=document.getElementById('lineageInfo'); if(info) info.textContent=`Ancestor chain length ${chain.length}. Yellow dots = mutation. Selected gen ${org.generation} with ${Object.keys(org.mutations).length} mutations.`;
  }
  saveToStorage(){
    const data={
      version:'1.0', time:Date.now(), environment:this.environment, controls:this.controls,
      organisms:this.organisms.filter(o=>o.alive).map(o=>({id:o.id, genome:o.genome, x:o.x, y:o.y, vx:o.vx, vy:o.vy, energy:o.energy, health:o.health, age:o.age, parentIds:o.parentIds, generation:o.generation, lineageId:o.lineageId, role:o.role, mutations:o.mutations, isEgg:o.isEgg, eggTimer:o.eggTimer})),
      habitats:this.habitats, research:{traitsDiscovered:[...this.research.traitsDiscovered], totalBirths:this.research.totalBirths, symbiosisObserved:this.research.symbiosisObserved},
      campaign:this.campaign.milestones, popHistory:this.popHistory, traitHistory:this.traitHistory, mode:this.mode, settings:this.settings
    };
  ...
  load(){
    ...
    try{
      ...
      this.habitats=data.habitats||[]; this.research.traitsDiscovered=new Set(data.research.traitsDiscovered||[]); this.research.totalBirths=data.research.totalBirths||0; this.research.symbiosisObserved=data.research.symbiosisObserved||false;
      if(data.campaign) this.campaign.milestones=data.campaign;
      this.popHistory=data.popHistory||this.popHistory; this.traitHistory=data.traitHistory||this.traitHistory; this.mode=data.mode||'campaign';
      if(data.settings) { this.settings=data.settings; this.saveSettings(); }
      const ids=['tempSlider','lightSlider','oxySlider','phSlider','nutSlider','filtSlider'];
      const keys=['temp','light','oxy','ph','nutrients','filt'];
      ids.forEach((id,i)=>{ const el=document.getElementById(id); if(el) el.value=this.controls[keys[i]]; });
...
window.addEventListener('load',()=>{
  sim=new Simulation(); window.sim=sim;
  window.runValidation=()=>Simulation.runValidation(sim);
  const urlParams=new URLSearchParams(window.location.search);
  if(urlParams.has('autotest')){
    setTimeout(()=>{
      sim.startGame('campaign');
      setTimeout(()=>{ sim.controls.temp=10; const el=document.getElementById('tempSlider'); if(el) el.value=10; },2000);
