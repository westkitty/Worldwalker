...
function mapPanel(){  const svgW=420,svgH=310,pt=p=>[20+p.x/WORLD.w*380,20+p.y/WORLD.h*260];const lines=[];
  ...
  for(const r of relationships){if(!state.expeditions[r.id]?.unlocked)continue;const a=projectOf(r.a),b=projectOf(r.b);if(!a||!b)continue;const [x1,y1]=pt(a),[x2,y2]=pt(b);lines.push(`<path d="M${x1} ${y1} L${x2} ${y2}" class="map-road secret"/>`)}
